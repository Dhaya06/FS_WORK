...
{
    public partial class View_Sales_Product : MetroFramework.Forms.MetroForm
    {

        int salesID = 0;
        string con = ConfigurationManager.ConnectionStrings["pos"].ConnectionString;
        public View_Sales_Product(int sid)
        {
     ...
     //       t.Abort();
        ...
        private void tblCart_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                int pid = Convert.ToInt32(tblCart.Rows[rw].Cells[0].Value.ToString());

                foreach (Form form in Application.OpenForms)
                {
                    if (form.GetType().Name == "ViewSalesProductQuantity")
                    {
                ...
                using (ViewSalesProductQuantity vsd = new ViewSalesProductQuantity(salesID, pid))
                {
                    vsd.ShowDialog();
                }
            ...
            {
                MetroMessageBox.Show(this, "! System Error . Code ERR_0847_Fatal_Error. " + ex.Message, "System Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
