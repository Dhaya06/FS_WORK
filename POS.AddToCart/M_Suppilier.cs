...
namespace POS.AddToCart
{
    public partial class M_Suppilier : Form
    {
        ...
        private void LoadCart(List<Supplier> sList)
        {
            try
            {
                int count = 0;
                ...
                foreach (var item in sList)
	            {
                    ...
                    dgvSupplier.Rows[count].Cells[4].Value = item.Company;
                    count++;
            ...
            try
            {
                ...
                if (cl == 5)
                {
                    ...
                    if (edited > 0)
                    {
                        ...
                        sp.Company = dgvSupplier.Rows[rw].Cells[4].Value.ToString();

                        if (sp.Update(con))
                        {
                            MetroMessageBox.Show(this, "Supplier Data Updated Successfully", "System Message", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);

                            PopupNotifier pop = new PopupNotifier();
                            pop.ContentText = "Supplier Data Updated Successfully";
                            pop.TitleText = "Saved";
                            pop.Image = Resources.success_48; // or Image.FromFile(--Path--)
                            pop.IsRightToLeft = false;
                            pop.ContentHoverColor = Color.OrangeRed;
